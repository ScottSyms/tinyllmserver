...
- **Chat** — any instruction-tuned **GGUF** model via [llama.cpp](https://github.com/ggml-org/llama.cpp) bindings (default: **Qwen3-8B**, 4-bit).
...
> e.g. a smaller model: `--model-repo unsloth/Qwen3-1.7B-GGUF --model-file Qwen3-1.7B-Q4_K_M.gguf`.
...
| `--model-repo` / `--model-file` | `unsloth/Qwen3-8B-GGUF` / `Qwen3-8B-Q4_K_M.gguf` | HF source |
...
```sh
./target/release/tinymodelserver --port 9000 --model ~/models/Qwen3-8B-Q4_K_M.gguf
```
...
curl http://127.0.0.1:8080/v1/chat/completions \
  ...
  -d '{"model":"qwen3-8b","messages":[{"role":"user","content":"Hi in French?"}]}'
...
c.chat.completions.create(model="qwen3-8b",
